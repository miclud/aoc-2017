...
const getRowDiff = (row) => {
  let smallest = 9999999999999999;
  let highest = 0;

  row.forEach(element => {
    const number = Number.parseInt(element, 10);
    if (number > highest) {
      highest = number;
    }
...
const processInput = (input, calculation) => {
  let sum = 0;
  input.forEach(row => {
    const split = row.split('\t');
    const diff = calculation(split);
    sum += diff;
  });
  return sum;
};

const getDivisibleNumber = (row) => {
  let sum = 0;
  ...
  row.forEach((element, index) => {
    for (var i = 0; i < numItems; i++) {
      ...
      let compareNum = row[i];
      const noDiff = element % compareNum === 0;
      if (noDiff) {
        sum = element / compareNum;
        break;
      }
    }
  });
...
const answer1 = processInput(input, getRowDiff);
console.log('Answer for part 1 is:', answer1); // => 32121

const answer2 = processInput(input, getDivisibleNumber);
console.log('Answer for part 2 is:', answer2); // => 197
